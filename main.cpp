...
constexpr int N = 1 << 12;  // 4096，坐标范围 [0, N)

// -------------------------------------------------------
// 稀疏数据结构：用哈希集合只存活细胞的坐标
// key = x * N + y，一个 int 就能唯一表示一个格子
// 相比原来 16M 个 bool，内存从 ~2MB 降到只存几千个 int
// -------------------------------------------------------
struct CellHash {
    // 乘以黄金比例哈希常数，减少冲突
...
using CellSet = std::unordered_set<int, CellHash>;
using CountMap = std::unordered_map<int, int, CellHash>;

CellSet cells;  // 只存活细胞

// 坐标 <-> key 互转
inline int  make_key(int x, int y) { return x * N + y; }
inline int  key_x(int k)           { return k / N; }
inline int  key_y(int k)           { return k % N; }

// -------------------------------------------------------
// 边界检查：与原代码保持一致，只允许 [1, N-2] 内的格子存活
// -------------------------------------------------------
...
void init(int bx, int by) {
    ...
    int x = bx;
    int y = by;
    for (int i = 0; gundata[i]; i++) {
        char c = gundata[i];
        if (strchr(" \n\t\\!", c)) continue;
        if ('0' <= c && c <= '9') { acc = acc * 10 + (c - '0'); continue; }
        if (!acc) acc = 1;
        if (c == 'b') {
            y += acc;  // 死细胞：仅推进 y 坐标，无需插入
        }
        if (c == 'o') {
            for (int o = 0; o < acc; o++) {
                if (in_bounds(x, y))
                    cells.insert(make_key(x, y));
                y++;
            }
        }
        if (c == '$') { y = by; x += acc; }
...
// -------------------------------------------------------
// step() 核心思路（替换掉原来 O(N²) 的全量遍历）：
//
//   1. 遍历所有活细胞，给其 8 个邻居的"邻居计数"加 1
//      → 只有可能存活的格子才会出现在 neigh_count 里
//
//   2. 遍历 neigh_count，应用 Conway 规则：
//      - 活细胞 && count==2||3 → 继续存活
//      - 死细胞 && count==3    → 变为活细胞
//      - 其余情况             → 死亡（不加入 next）
//
//   活细胞如果一个邻居都没有（count==0），根本不进 neigh_count，
//   自然就被淘汰，无需特判。
// -------------------------------------------------------
void step() {
    // reserve 减少 rehash 次数：每个活细胞最多影响 8 个格子
    CountMap neigh_count;
    neigh_count.reserve(cells.size() * 9);

    for (int k : cells) {
        int x = key_x(k), y = key_y(k);
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                if (!dx && !dy) continue;
                int nx = x + dx, ny = y + dy;
                if (in_bounds(nx, ny))
                    neigh_count[make_key(nx, ny)]++;
            }
        }
    }
    ...
    for (auto& [k, cnt] : neigh_count) {
        bool alive = cells.count(k);
